...
const COOKIE_TARGET_KEY = "ksjs_cookie_target";
...
async function main() {
  ...
  for (let i = 0; i < COOKIE_KEYS.length; i++) {
    ...
    if (!cookie) {
      notifyLines.push(`账号${i + 1}: Cookie未配置`);
      continue;
    }
    ...
    try {
      console.log(`\n===== 开始处理账号${i + 1} =====`);
      const result = await processAccount(cookie, i + 1);
      notifyLines.push(result);
      await $.wait(2000);
    } catch (e) {
      notifyLines.push(handleError(e, i + 1));
    }
  }

  const message = notifyLines.length ? notifyLines.join("\n\n") : "没有启用的账号";
  $.notify(NOTIFY_TITLE, "执行完成", message);
}
...
async function processAccount(cookie, accountNum) {
 ...
 if (boxRes.success) {
  ...
  const msg = [
    `账号${accountNum}: ${initialInfo.nickname}`,
    `签到状态: ${checkinRes}`,
...
function getAvailableAccountSlot() {
  const target = parseInt($.read(COOKIE_TARGET_KEY) || "0", 10);
  if (target >= 1 && target <= COOKIE_KEYS.length) return target;
...
function handleError(e, accountNum) {
  ...
  if (/登录|auth|鉴权|验证|未登录|身份|403|302/i.test(msg)) {
    ...
    return `账号${accountNum}: Cookie失效，请重新获取Cookie`;
  } else {
    return `账号${accountNum}: 执行错误\n${msg}`;
  }
}
